...
import {apiComments, CommentType} from "features/Comments/commentsApi";


export const fetchCommentsTC = createAsyncThunk('Posts/fetchComment', async (param: { postId: string }, {
  dispatch,
  rejectWithValue,
  getState
}) => {
...
export const createCommentTC = createAsyncThunk('Posts/createComment', async (param: { content: string, postId: string }, {
  dispatch,
  rejectWithValue
}) => {
  try {
