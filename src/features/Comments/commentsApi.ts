import {LikeInfoType} from "features/Posts/postsApi";
...
export const apiComments = {
  ...
  getComments(postId: string, data: {pageNumber: number, pageSize: number}) {
    return instance.get<ResponseType<CommentType[]>>(`api/posts/${postId}/comments`, {params: data})
      .then(res => res.data)
  },
  createComment(data: { content: string }, postId: string) {
    return instance.post<'', AxiosResponse<CommentType>, {content: string}>(`api/posts/${postId}/comments`, data)
      .then(res => res.data)
  },
